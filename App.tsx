...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'home': return <Home lang={lang} t={t} onNavigate={setActiveTab} />;
      ...
      default: return <Home lang={lang} t={t} onNavigate={setActiveTab} />;
    }
  };
      ...
      <div className="relative flex flex-col items-center justify-center h-screen overflow-hidden bg-black font-sans">
        ...
        <div className="relative z-10 w-full max-w-sm px-6 text-center space-y-8 animate-in fade-in duration-700">
          <div className="p-10 bg-slate-900/50 backdrop-blur-3xl rounded-[3.5rem] shadow-2xl border border-white/10 space-y-8">
            ...
            <p className="text-lg text-white font-bold italic leading-tight">{t.apiKeyDesc}</p>
    ...
    <div className={`flex flex-col h-screen font-sans ${darkMode ? 'bg-slate-950 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <header className={`p-4 shadow-xl sticky top-0 z-50 ${darkMode ? 'bg-slate-900 border-b border-slate-800' : 'bg-white border-b border-gray-100'}`}>
        <div className="flex items-center justify-between max-w-3xl mx-auto">
          ...
          <button onClick={() => { hapticTap(); setActiveTab('settings'); }} className="p-2.5 rounded-xl bg-gray-100 dark:bg-slate-800">
      ...
      <nav className={`fixed bottom-0 left-0 right-0 border-t p-2 z-50 ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-gray-100'}`}>
        <div className="max-w-3xl mx-auto flex justify-around items-center h-20">
          <NavItem active={activeTab === 'home'} icon="home" label={t.home} onClick={() => setActiveTab('home')} />
          <NavItem active={activeTab === 'fact-checker'} icon="fact_check" label={t.factChecker} onClick={() => setActiveTab('fact-checker')} />
          <NavItem active={activeTab === 'office-locator'} icon="location_on" label={t.iebcLocator} onClick={() => setActiveTab('office-locator')} />
          <NavItem active={activeTab === 'learn'} icon="school" label={t.learn} onClick={() => setActiveTab('learn')} />
          <NavItem active={activeTab === 'assistant'} icon="voice_chat" label={t.assistant} onClick={() => setActiveTab('assistant')} />
        </div>
      </nav>
    </div>
  );
...
const NavItem: React.FC<{ active: boolean; icon: string; label: string; onClick: () => void }> = ({ active, icon, label, onClick }) => (
  <button onClick={() => { hapticTap(); onClick(); }} className={`flex-1 flex flex-col items-center justify-center py-2 rounded-2xl ${active ? 'bg-[#135bec]/10 text-[#135bec]' : 'text-gray-400'}`}>
